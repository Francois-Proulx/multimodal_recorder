...
class Audio_Adq(Node):
    def __init__(self):
        ...
        self.audio_path = (
            self.get_parameter("audio_path").get_parameter_value().string_value
        )
        ...
        # --- Publisher ---
        self.publisher = self.create_publisher(AudioRaw, "/audio_raw", 20)

        # --- Choose playback (csv) or live acquisition thread ---
    ...
    def from_csv(self):
        ...
        if self.audio_path and os.path.exists(self.audio_path):
            ...
            self.get_logger().info(
                "max data : " + str(np.max(data)) + ". min data = " + str(np.min(data))
            )
        else:
        ...
        for index, row in csv_reader.iterrows():
            ...
            this_data = np.reshape(
                data[t_ini:t_fin, :], win_len * channels, order="F"
            ).tolist()
            self.get_logger().info(
                "max data : "
                + str(max(this_data))

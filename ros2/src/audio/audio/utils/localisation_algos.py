...
def SRP_PHAT_trad(YYs_PHAT, TDOAs_scan, f):
    ...
    W = SRP_PHAT_offline(
        TDOAs_scan, nb_of_channels, f
    )  # [nb_of_doas, nb_of_pairs*nb_of_bins]
    ...
    SRP = np.zeros(
        (nb_of_frames, nb_of_doas), dtype=np.float32
    )  # [nb_of_frames, nb_of_doas]
    for frame_id in tqdm(range(nb_of_frames), desc="SRP-PHAT frames"):
        ...
        ].T.flatten()  # [nb_of_pairs*nb_of_bins,]
...
def SRP_PHAT_GCC(YYs_PHAT, TDOAs_scan, fs, q=1, QUAD_INTERP=False):
    ...
    if QUAD_INTERP:  # (float values)
        ...
        tau_diff_samples_int = (
            np.rint(tau_diff * FS_INTERP).astype(np.int32) % FRAME_SIZE_ZERO_PAD
        )
    ...
    for frame_id in tqdm(range(nb_of_frames), desc="SRP-PHAT-GCC frames"):
        ...
        if QUAD_INTERP:
            ...
            rho_selected = np.take_along_axis(
                rho_pairs, tau_diff_samples_int, axis=1
            )  # [nb_of_pairs, nb_of_doas]
...
def SRP_PHAT_FCC(YYs_PHAT, TDOAs_scan, f, delta=0.01, K=None):
    ...
    N = (nb_of_bins - 1) * 2
    nb_of_pairs = (
        nb_of_channels * (nb_of_channels - 1) // 2
    ...
    for frame_id in tqdm(range(nb_of_frames), desc="SRP-PHAT-FCC frames"):
        ...
        SCM_eff_s = (
            YY_pairs[:, first_half_indices] + YY_pairs[:, second_half_indices]
        )  # [nb_of_pairs, N/4]
        SCM_eff_s = np.concatenate(
            (SCM_eff_s, np.expand_dims(YY_pairs[:, N // 4], axis=1)), axis=1
        )  # [nb_of_pairs, N/4+1]

        SCM_eff_d = (
            YY_pairs[:, first_half_indices] - YY_pairs[:, second_half_indices]
        )  # [nb_of_pairs, N/4]
        SCM_eff_d = np.concatenate(
            (SCM_eff_d, np.expand_dims(YY_pairs[:, N // 4], axis=1)), axis=1
        )  # [nb_of_pairs, N/4+1]
        ...
        SRP[frame_id, :] = np.sum(
            SRPs, axis=0
        )  # Sum over pairs to get SRP for each DOA [nb_of_doas]
...
def SVD_MUSIC(SCM_XX, SCM_VV, TDOAs_scan, f):
    ...
    for frame_id in tqdm(range(nb_of_frames), desc="SVD-MUSIC frames"):
        ...
        SCM_VV_loaded = shrinkage_diagonal_loading(
            SCM_VV_frame, shrinkage_intensity=0.01
        )
...
def GCCF(YYs):
    ...
    yys = np.zeros(
        (nb_of_channels, nb_of_channels, nb_of_frames, frame_size), dtype=np.float32
    )
...
def GCC_PHAT(YYs, eps=1e-20, FRAME_SIZE_INTERP=4096):
    ...
    yys_interp = np.zeros(
        (nb_of_channels, nb_of_channels, nb_of_frames, FRAME_SIZE_INTERP)
    )
    taus = np.zeros((nb_of_channels, nb_of_channels, nb_of_frames))
    for ii in range(nb_of_channels):
        for jj in range(nb_of_channels):
            ...
            taus[ii, jj, :] = np.argmax(yys[ii, jj, :, :], axis=1)
            yys_interp[ii, jj, :, :] = np.fft.irfft(
                YY / (np.abs(YY) + eps), n=FRAME_SIZE_INTERP
            )
...
def SRP_PHAT_offline(TDOAs_scan, nb_of_channels, f):
    ...
    TDOAs_scan = np.tile(
        TDOAs_scan[:, :, np.newaxis], (1, 1, nb_of_bins)
    )  # [nb_of_channels, nb_of_doas, nb_of_bins]
    ...
    for chan_id1 in range(nb_of_channels):
        ...
        for chan_id2 in range(chan_id1 + 1, nb_of_channels):
            # Wj = e^(jomega tau_j)
            Wj = np.exp(
                1j * omega * TDOAs_scan[chan_id2, :, :]
            )  # [nb_of_doas, nb_of_bins]
...
def SRP_from_GCC_PHAT(yys, TDOAs_scan, fs):
    ...
    for chan_id1 in range(nb_of_channels):
        for chan_id2 in range(chan_id1 + 1, nb_of_channels):
            ...
            R_ij = yys[chan_id1, chan_id2, :, :].astype(
                np.float32
            )  # R_ij > 0 if i measured before j [nb_of_frames, frame_size]
...
def SVD_PHAT_online(XX, D, Vh_k, nb_of_sources=2):
    ...
    for source_id in range(nb_of_sources):
        ...
        if norm_ur > 0:
            ...
            raise ValueError(
                f"Zero norm encountered at iteration {source_id}, check input data."
            )
...
def FCC_PHAT_offline(TDOAs_scan, f, delta=0.01):
    ...
    for chan_id1 in range(nb_of_channels):
        ...
        for chan_id2 in range(chan_id1 + 1, nb_of_channels):
            ...
            U_all[pair_idx], s_all[pair_idx], Vh_all[pair_idx] = np.linalg.svd(
                Wij, full_matrices=False
            )

            # compute cumulative energy ratio
            energy = np.cumsum(s_all[pair_idx] ** 2) / np.sum(s_all[pair_idx] ** 2)
            Ks[pair_idx] = (
                np.searchsorted(energy, (1 - delta)) + 1
    ...
    Bs = np.zeros(
        (nb_of_pairs, K_global, N // 4 + 1), np.cdouble
    )  # [nb_of_pairs, nb_of_doas, N/4+1]
    U_ks = np.zeros(
        (nb_of_pairs, nb_of_doas, K_global), np.cdouble
    )  # [nb_of_pairs, nb_of_doas, K]
...
def FCC_PHAT_offline_known_K(TDOAs_scan, f, K):
    ...
    Bs = np.zeros(
        (nb_of_pairs, K, N // 4 + 1), np.cdouble
    )  # [nb_of_pairs, nb_of_doas, N/4+1]
    U_ks = np.zeros(
        (nb_of_pairs, nb_of_doas, K), np.cdouble
    )  # [nb_of_pairs, nb_of_doas, K]
    deltas = np.zeros((nb_of_pairs), np.float32)
    pair_idx = 0
    for chan_id1 in range(nb_of_channels):
    ...
    for frame_id in range(nb_of_frames):
        ...
        for chan_id1 in range(nb_of_channels):
            for chan_id2 in range(nb_of_channels):
                ...
                yys_ij_frame = yys_shifted[
                    chan_id1, chan_id2, frame_id, :
                ]  # [frame_size]

                # Theoretical taus
                TDOA_diff_SRP = (
                    int(np.round(TDOAs_max_SRP[chan_id1] - TDOAs_max_SRP[chan_id2]))
                    + frame_size // 2
                )
                ...
                while left_neg_gradient:
                    ...
                    if left_val <= ref_val:
                        yys_modified_shifted[
                            chan_id1, chan_id2, frame_id, yys_id - 1
                        ] = 0
                        yys_id -= 1
                    else:
...
def GEVD_MUSIC(
    SCM_YY, SCM_VV, TDOAs_Scan, f, nb_of_sources=0, eigenvalue_threshold=100
):
    """
    ...
    SCM_VV_loaded = shrinkage_diagonal_loading(
        SCM_VV, shrinkage_intensity=0.01
    )  # [nb_of_channels, nb_of_channels, nb_of_bins]
    ...
    for bin_id in range(nb_of_bins):
        ...
        Qv = eig_vec[
            :, nb_of_sources:
        ]  # [nb_of_channels, nb_of_channels-nb_of_sources]

        # DOA candidate vector
        b = np.exp(
            -1j * 2 * np.pi * f[bin_id] * TDOAs_Scan
        )  # [nb_of_channels, nTheta, nPhi]
...
def MUSIC_offline(TDOAs_Scan, f):
    ...
    b = np.exp(
        -1j * omega * TDOAs_Scan[np.newaxis, :, :]
    )  # [nb_of_bins, nb_of_channels, nb_of_doas]
...
def GSVD_MUSIC_vec(SCM_YY, SCM_VV, b, nb_of_sources=0, singlevalue_threshold=2):
    ...
    Mat = np.einsum(
        "ijk,ikl->ijl", SCM_VV_inv, SCM_YY
    )  # [nb_of_bins, nb_of_channels, nb_of_channels]
    mat_calc = time.time()

    # Perform SVD on Mat for all frequency bins
    Ql, eig_vals, _ = np.linalg.svd(
        Mat
    )  # Ql: [nb_of_bins, nb_of_channels, nb_of_channels], eig_vals: [nb_of_bins, nb_of_channels]
...
def localisation_mvdr_vec(SCM_YY, SCM_VV, TDOAs_Scan, f, eps=1e-6):
    ...
    P_mvdr_freq = np.abs(
        np.einsum("bmd,bmn,bnd->bd", np.conj(b), Mat, b)
    )  # [nb_of_bins, nb_of_doas]

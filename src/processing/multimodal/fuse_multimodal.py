...
def trim_audio_to_imu_and_video(audio_fft_timestamps, t_imu, t_video, doas, doas_coord):
    ...
    if audio_start < last_start:
        n_trim = min(
            int(np.ceil((last_start - audio_start) / hop_size)),
    ...
    if audio_end > first_end:
        n_trim = min(
            int(np.ceil((audio_end - first_end) / hop_size)), len(audio_fft_timestamps)
        )
...
def fuse_audio_video(
    ...
    audio_fft_timestamps, doas_trad, doas_coord = trim_audio_to_imu_and_video(
        audio_fft_timestamps,
        imu_timestamps,
        video_timestamps,
        DOAs_trad,
        DOAs_coord_trad,
    ...
    quat_imu_interp, R_imu = interpolate_quaternions(
        quats_imu, imu_timestamps, audio_fft_timestamps
    )
    ...
    plot_doa_and_yaw(
        ...
        ids=["imu yaw", "apriltag yaw"],
        title="SSL and sensors yaw",
    )
    ...
    DOAs_trad_world_imu = DOAs_from_DOAs_coordinates(
        DOAs_coord_world_imu, center_around_zero=True
    )

    DOAs_coord_world_video = apply_rotation(
        R_video, doas_coord
    )  # np.linalg.inv(R_video)
    DOAs_trad_world_video = DOAs_from_DOAs_coordinates(
        DOAs_coord_world_video, center_around_zero=True
    )

    # plot_DOAs_single_source(DOAs_trad_world, t, filename=None)
    plot_doa_world(
        t,
        doas_world=[DOAs_trad_world_imu, DOAs_trad_world_video],
        doas_ids=["DOAs imu", "DOAs apriltag"],
        sensors=[yaw_imu, yaw_video],
    ...
    return (
        DOAs_trad_world_imu,
        DOAs_coord_world_imu,
        DOAs_trad_world_video,
        DOAs_coord_world_video,
    )
...
if __name__ == "__main__":
    ...
    AUDIO_FILE = project_file(
        "data", "raw", "multimodal", "session_test_2", "audio", "audio.wav"
    )
